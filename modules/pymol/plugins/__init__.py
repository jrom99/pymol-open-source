...
startup_path = [cmd.exp_path('$PYMOL_DATA/startup')]
# API functions
...
def pref_save(filename=PYMOLPLUGINSRC, quiet=1):
    ...
    text = f"""
        ...
        try:,
          import {__name__},
          {__name__}.autoload = {autoload!r},
          {__name__}.preferences = {preferences!r},
          {__name__}.set_startup_path({get_startup_path(True)!r}, False),
        except:,
          import os,
          print("Error while loading", os.path.abspath(__script__)),
    """
    try:
        with open(cmd.exp_path(filename), "w") as f:
            f.write(dedent(text))
    except IOError:
        print(f' Plugin-Error: Cannot write Plugins resource file to {filename!r}')
        return
...
def initialize(pmgapp=-1):
    ...
    modules = findPlugins(startup_path)
